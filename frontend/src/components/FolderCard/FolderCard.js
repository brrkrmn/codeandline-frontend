...
const FolderCard = ({ folder, empty }) => {
  if (empty) {
    return (
      <Link to='/create/folder' className="flex flex-col w-[350px] h-[180px] p-4 grow-0 shrink-0 border-1 rounded-xl border-1 border-border shadow-small transition hover:drop-shadow-sm">
    ...
    return (
      <Link to={!folder.mock && `/folder-overview/${folder.id}`} className="flex flex-col w-[350px] h-[140px] p-4 grow-0 shrink-0 border-1 rounded-xl border-1 border-border shadow-small transition hover:drop-shadow-sm">
